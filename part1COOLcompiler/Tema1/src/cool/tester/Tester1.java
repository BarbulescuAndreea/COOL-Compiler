...
public class Tester1 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Arrays.sort(files);
        File[] remainingFiles = new File[18];
        // Check if files is not null and has at least two elements
        if (files != null && files.length >= 9) {
            // Create a sublist starting from the third element (index 2)
            remainingFiles = Arrays.copyOfRange(files, 13, files.length);

            // Now, remainingFiles contains the elements from index 2 to the end
        }
        for (var file : files) {
